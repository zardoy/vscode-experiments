...
export const registerAutoCloseTag = () => {
    registerWithSetting(['features.jsxAutoCloseTag'], ({ featuresJsxAutoCloseTag }) => {
        if (!featuresJsxAutoCloseTag) return
        const lastTwoChanges = [] as string[]
        return vscode.workspace.onDidChangeTextDocument(async ({ contentChanges, document, reason }) => {
